#!/usr/bin/env bash
# cassegrain_flat: run_cassegrain.sh with --flat-mirrors -- identical pointing,
# Zernike c3/c4/c5 forced to zero. See that script's header for every other
# decision (design numbers, hand-built model requirements, verification);
# see run_axicon_flat.sh for the union-form scalars.

set -u

REPO="C:/gitlab/heliostats"
NAME="cassegrain_flat"
...
if [ ! -f "$MODEL" ]; then
    say "$MODEL does not exist -- see run_cassegrain.sh's header. NOT starting."
    exit 1
fi
